...
// @Success     200 {object} GetUsersResponse                 "Успешное получение спписка пользователей"
...
func getLimitOffsetQueryParams(r *http.Request) (*int, *int, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("limit query param: %w", err)
	}

	offset, err := core_http_request.GetIntQueryParams(r, offsetQueryParamKey)
	if err != nil {
		return nil, nil, fmt.Errorf("offset query param: %w", err)
	}
